...
func routeMentionedGroupMessage(req seatalk.EventRequest, event *seatalk.NewMentionedMessageReceivedFromGroupChatEvent) (seaTalkRoute, bool, error) {
	...
	case "interactive_message":
		kind = agent.MessageKindInteractiveCard
		text = strings.TrimSpace(summarizeInteractiveMessage(event.Message.InteractiveMessage))
		imageURLs = compactURLs(seatalk.ExtractInteractiveImageURLs(event.Message.InteractiveMessage))
	...
	return seaTalkRoute{
		message: agent.InboundMessage{
			...
			Kind:            kind,
			Sender:          formatCurrentMessageSender(event.Message.Sender.Email),
...
func normalizeMessageContent(
	...
	case "interactive_message":
		return agent.MessageKindInteractiveCard,
			strings.TrimSpace(summarizeInteractiveMessage(interactiveMessage)),
			compactURLs(seatalk.ExtractInteractiveImageURLs(interactiveMessage)),
			true
	default:
		return "", "", nil, false
	}
...
func formatCurrentMessageSender(email string) string {
	email = strings.TrimSpace(email)
	if email == "" {
		return "unknown"
	}
...
func extractForwardedSender(value any) string {
	...
	for _, candidate := range []string{"email", "employee_code", "seatalk_id"} {
		if resolved := strings.TrimSpace(stringValue(fields[candidate])); resolved != "" {
			return resolved
		}
	}
