...
func TestNewAppServerRunnerDefaultsReasoningToMedium(t *testing.T) {
	...
	expectedSandboxPolicy := SandboxPolicyReadOnly
	...
	if !reflect.DeepEqual(runner.resumeOptions.Config, expectedConfig) {
		t.Fatalf("unexpected default resume config: %#v", runner.resumeOptions.Config)
	}
}
...
func TestNewAppServerRunnerPreservesExplicitConfigOverrides(t *testing.T) {
	t.Parallel()
