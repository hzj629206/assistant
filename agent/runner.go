...
func (r *NoopRunner) RunTurn(ctx context.Context, req TurnRequest) (TurnResult, error) {
	...
	result := TurnResult{}
	if strings.TrimSpace(firstDebugCommand(req.Message)) == "/debug" {
		payload, err := r.debugPayload(req)
		if err != nil {
			return result, err
		}
		result.ReplyText = payload
		return result, nil
	}

	if len(mergedMessages) > 0 {
		result.ReplyText = latestTextReply(mergedMessages)
		if result.ReplyText != "" {
	...
	if req.Message.Kind == MessageKindText {
		result.ReplyText = req.Message.Text
	}
	if req.Message.Kind == MessageKindForwarded {
		result.ReplyText = "<forwarded_messages>"
	}
	if req.Message.Kind == MessageKindImage {
		result.ReplyText = "<image>"
	}
...
func (r *NoopRunner) debugPayload(req TurnRequest) (string, error) {
	...
	body, err := json.MarshalIndent(noopRunnerDebugPayload{
		Runner:        "noop",
