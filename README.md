...
For outbound assistant replies, interactive cards are also a preferred presentation format when the assistant needs to report intermediate status or progress for a complex task,
or when it needs to present complex results in a structured way instead of relying only on plain text.
...
3. Run this project. If the service runs on a local machine without public inbound access, use SSH forwarding so the public `Nginx` host can reach the callback endpoint.
...
4. Configure the SeaTalk callback URL as `https://<domain>/callback` in the SeaTalk platform.
...
```bash
go install github.com/hzj629206/assistant/cmd/codexd
```
...
Pass runtime options directly when starting the service:

```bash
codexd \
  -listen-addr :8421 \
  -seatalk-app-id your-app-id \
  -seatalk-app-secret your-app-secret \
  -seatalk-signing-secret your-signing-secret
```

Common flags: `codexd -h`

Security note: the Codex CLI backend runs in `read-only` sandbox mode with `never` approval by default.
Do not store sensitive data in the working directory because the model may still be able to read files from that directory.

### 2. Use The Default Config File

If `$HOME/.assistant/config.yml` exists, the service loads it automatically.
...
Pass a YAML config file with `-config` or `-f` to override the default path.
